...
fn should_create_session() {
    let db = test_db();
    let id = db.create_session("myproject", "/home/user/myproject").unwrap();
    assert!(id > 0);
}
...
fn should_get_latest_session_for_project() {
    let db = test_db();
    db.create_session("myproject", "/home/user/myproject").unwrap();
    let id2 = db.create_session("myproject", "/home/user/myproject").unwrap();
